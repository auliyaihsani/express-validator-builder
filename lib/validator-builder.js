...
class ValidatorBuilder {
  ...
  mapRules (fieldPath, rule) {
    ...
    switch (rule) {
      ...
      case 'isUUID':
        validation[rule] = this.mapTemplate(rule, {
          version: this.options.uuidVersion,
          path: fieldPath
        }); break
  ...
  mapTemplate (rule, options) {
    let result = {}
    let tmp = templates[rule] || {}; if (_.isEmpty(tmp)) return tmp
    
    if (!_.isEmpty(tmp.options)) {
      Object.assign(result, {options: []})
      
      tmp.options.forEach(key => {
        if (!_.isNil(options[key])) {    
          result.options.push(options[key])
        }
      })
    }

    return Object.assign(result, {
      errorMessage: sprintf(tmp.errorMessage, options)
    })
  }
  ...
  fecthCustomKeys (customValidators) {
